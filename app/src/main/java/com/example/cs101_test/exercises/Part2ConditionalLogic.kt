...
object Part2ConditionalLogic {
    ...
    fun sumDigits(number: Int): Int {
        ...
        for (number in string) {
            sum += number.digitToInt() // Søgt på google
        }
        return sum
    }
    ...
    fun transformToLowercase(strings: List<String>): List<String> {
        return strings.map { it.lowercase() } // Hjælp fra chatGPT
    }
    ...
    fun isPalindrome(input: String): Boolean {
        val clean = input.replace(" ", "") // Hjælp fra chatGpt
    ...
    fun calculateArea(width: Int = 2, height: Int = 3): Int {
        return width * height
    }
