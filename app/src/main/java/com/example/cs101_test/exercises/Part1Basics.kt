...
object Part1Basics {
    ...
    fun secondLargest(list: List<Int>): Int {
        val secondLargestInList = list.sortedDescending()[1]
        return secondLargestInList
    }
    ...
    fun findMax(numbers: List<Int>): Int {
        return numbers.maxOrNull()!! // Brugt chatGPT - Virker stadigvæk ikke
        // Nok fordi man ikke kan finde max i en liste.
    }
    ...
    fun charFrequency(str: String): Map<Char, Int> {
        ...
        for (char in str)
            if (char.isLetter()) {
                map[char.lowercaseChar()] = (map[char.lowercaseChar()] ?: 0) + 1 // Hjælp fra chatgpt
            }
